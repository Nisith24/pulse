...
fun CustomListDetailScreen(
    ...
    val lectures by viewModel.currentListLectures.collectAsState()
    ...
    LaunchedEffect(listId) {
        viewModel.loadLecturesForList(listId)
    }
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (lectures.isEmpty()) {
                item {
                    Text(
                        "Playlist is empty. Long-press on any video to add it here.",
                ...
                items(lectures, key = { it.id }) { lecture ->
                    LectureCard(
                        ...
                        onToggleFavorite = { /* Not fully connected back to LibraryVM here */ },
                        onLongPress = {
    ...
    selectedLongPressLecture?.let { lecture ->
        LectureActionBottomSheet(
            ...
            onDownloadClick = { /* repository direct call might be needed or add to this VM */ },
            ...
            onRemoveFromListClick = { viewModel.removeLectureFromList(listId, lecture.id) }
        )
    }
}
